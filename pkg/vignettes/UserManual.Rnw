...
Common or "vernacular" names are also available on the EOL pages and their associated languages. If output is set to detail (or d), it will return a large data frame with all of the bits of information. These tables may contain NAs, because not all information will be available for every taxon.  If output=counts, then a simple data frame of counts will be retuned without all the specific data.

<<>>=
CommonNames <- GetCommonNames(MyEOLs, output="c")
@
...
<<>>=
References <- GetReferences(MyEOLs, output="c")
@
...
<<>>=
ProviderData <- GatherProviderDataFrame(MyEOLs)
@

 
 future work to add bibtex stuff here for references




\section{Downloading Provider Pages}
add all the downloading stuff



 \section{Gathering Data from Provider Pages}
add synonyms and taxonomy stuffs



\section{Creating a Taxonomic Dendrogram}
add tree stuff 
 


\begin{center} \texttt{example(congruify.phylo)} \end{center}  
