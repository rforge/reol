DownloadEOLpages <- function(pages, to.file=TRUE, MyKey=NULL, verbose=TRUE) {
  if(Sys.getlocale("LC_ALL") == "C")
    paste("Sys.getlocale is set to C. In order to read UTF characters, you need to set the locale aspect to UTF-8 using Sys.setlocale")
  ...
  for (i in sequence(length(pages))) {
    ...
    if(to.file) {
      pageData <- getURL(web)
      if(length(grep("error", pageData)) == 0){
        write(pageData, file=paste("eol", pages[i], ".xml", sep=""))
        if(verbose)
